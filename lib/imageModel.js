...
function initTable() {
    ...
    // 既存テーブルに generation_time_ms カラムがなければ追加
    try {
        const columns = db.all("PRAGMA table_info(images)");
        const hasGenTime = columns.some(c => c.name === 'generation_time_ms');
        if (!hasGenTime) {
...
module.exports = {
    ...
    findImages(options = {}) {
        ...
        if (keyword && keyword.trim() !== '') {
            sql += ' WHERE prompt LIKE ? OR negative_prompt LIKE ?';
        ...
        const countParams = [];
        if (keyword && keyword.trim() !== '') {
            countSql += ' WHERE prompt LIKE ? OR negative_prompt LIKE ?';
            const k = `%${keyword.trim()}%`;
    ...
    createImage(data) {
        ...
        const sql = `
            INSERT INTO images (
                prompt, negative_prompt, width, height, steps, cfg_scale, seed, sampler_name,
